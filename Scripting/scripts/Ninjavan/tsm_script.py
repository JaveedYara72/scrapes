...
cred_email = ["lester.teng+sg@una-brands.com","lester.teng@una-brands.com","jujuadmin@juju.com.au"]
cred_pass = [ "7uKa2RGaB!trDmr","7uKa2RGaB!trDmr","E4JaJ8wGB9o@WR"]
cred_file_name = ["tsm_sg","tsm_my","juju_my"]

def rename_file(download_path,cred_file_name):
    ...
    # logic for file renaming -> if it already exists, read it into a df
    # put df into an s3
    # otherwise just put the entire thing into a df and push it into s3
    # write logic here for if exists
    # or maybe we remove all the files once they get downloaded.
    # just to be safe, write and if exists code here
    newest_file = files[-1]
    if "gecko" in newest_file:
        newest_file = files[-2]

    os.rename(newest_file, cred_file_name + ".csv")

def read_file(download_path, cred_file_name):
    os.chdir(download_path)
    files = filter(os.path.isfile, os.listdir(download_path))
    files = [os.path.join(download_path, f) for f in files]  # add path to each file
    files.sort(key=lambda x: os.path.getmtime(x))
    ...
    # check substring, if its geckodriver then skip
    # if not, then change it to something else 
    newest_file = files[-1]
    if "gecko" in newest_file:
        newest_file = files[-2]
    
    if "Adhoc" in newest_file:
        pass
    else:
        print("The file is not an adhoc, please check the script and run again")
        print("Quitting the process")
        quit()

    print(f"The newest file is -> {newest_file}" )
    dataframe_ = pd.read_csv(newest_file)
    print(dataframe_)

    # return dataframe, if you want to catch it in a function outside
...
# Warehouse Report                                                                         
    ...
    # # renaming the file
    # print("runnning the renaming script")
    # try:
    #     rename_file(download_path,cred_file_name)
    # except Exception as error:
    #     print(f"{error} ->Error while renaming the file. The file might/might not exist in the download folder")

    # read the existing file into a df
    print("Reading the Excel file into a dataframe")
    read_file(download_path,cred_file_name)
...
for i in range(len(cred_email)):
    download_path = r"C:\Users\Y Javeed\Downloads\Una Brands\Script_test"
    options = Options()
